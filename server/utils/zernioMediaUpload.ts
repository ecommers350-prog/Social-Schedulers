...
const debugLog = (
  ...
  // #region agent log
  fetch(
    "http://127.0.0.1:7528/ingest/672da1b7-1ec2-4b96-b90f-08a26a88d868",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Debug-Session-Id": "b6a79a",
      },
      body: JSON.stringify({
        sessionId: "b6a79a",
        location,
...
export const uploadBufferToZernio = async (
  ...
  // #region agent log
  debugLog(
    "zernioMediaUpload.ts:presign:start",
    "requesting Zernio presigned URL",
    {
      filename,
      contentType,
      size: buffer.length,
  ...
  // Convert Buffer to Blob (fixes TS2769)
  ...
  if (!putResponse.ok) {
    throw new Error(
      `Zernio media PUT failed with status ${putResponse.status}`
    );
  }

  // #region agent log
  debugLog(
    "zernioMediaUpload.ts:presign:success",
    "uploaded media to Zernio",
    {
      hasPublicUrl: !!presignData.publicUrl,
...
export const resolvePublicMediaUrl = async (
  ...
  // #region agent log
  debugLog(
    "zernioMediaUpload.ts:resolve:start",
    "re-uploading local media to Zernio",
    {
      filename,
